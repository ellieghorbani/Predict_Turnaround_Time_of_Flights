...
def fit_model(model_type, name_model, features_data, target_data, rand_state,name_target):
    ...
    plt.title('DL Airlines - {}'.format(name_target), fontdict=fontdict, color="black")             
    plt.savefig("feature_importance_DL_{}_predict_{}.png".format(name_model, name_target), bbox_inches='tight')
...
def plot(name_model,y_test, y_test_predict,name_target):
    ...
    ax.hist(y_test,bins = 100, alpha = 0.5, label = (name_target+' - CV'))
    ax.hist(y_test_predict, bins = 100,alpha = 0.5, label = name_target+' - PCV')
    ax.set(xlabel = 'Turnaround time (h)')
    ax.set(ylabel = 'Historam')
    ax.legend()
    plt.title('DL Airlines - {}'.format(name_target), fontdict=fontdict, color="black") 
    plt.savefig("histogram_DL_{}_predict_{}.png".format(name_model, name_target), bbox_inches='tight')
